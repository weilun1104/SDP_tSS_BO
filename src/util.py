...
def select_training_set(
    ...
    # print(sortargs)
    # print("*"*50)
    sortargs = torch.clamp(sortargs, 0, 22)
...
def sample_model_gradient(
    ...
    X_list = torch.cat([X_list + delta * torch.eye(n_dim).to(device), X_list - delta * torch.eye(n_dim).to(device)], 0)
    ...
    with torch.no_grad():
        for i in range(size_list):
            ...
            Y_sample = torch.cat([Y_sample, model_list[i].likelihood(mvn).sample(torch.Size([1])) * (y_std[i].to('cpu') + 1e-6) + y_mean[i].to('cpu')], 0)
    if size_list > 1:
...
def select_candidate_EI_unconstrained(
    ...
    X_torch = X
    X_candidate_torch = torch.tensor(X_candidate).to(device)
...
def select_candidate_TS_unconstrained(
    ...
    with torch.no_grad():
        ...
        y_cand = model.likelihood(y_cand_dist).sample(torch.Size([batch_size])).t().to(device).detach()
...
def select_candidate_TS_constrained(
    X,
    Y,
    X_candidate,
    batch_size,
    ...
    # 计算目标函数值和约束值的均值与标准差，用于后续的归一化处理
    y_mean, y_std = Y_torch.mean(0), Y_torch.std(0)

    # 归一化目标函数值和约束值
    Y_torch = (Y_torch - y_mean) / (1e-6 + y_std)

    # 创建多个GP模型，分别针对目标函数和每个约束
    models = []
    for i in range(Y_torch.shape[1]):
        model = GP({'train_x': X_torch, 'train_y': Y_torch[:, i:i+1], 'train_sigma': None}, kernel='MAT52')
        model.fit()
    ...
    # 使用 ExitStack 来控制 GPyTorch 的计算上下文
    with ExitStack() as es:
        if sampler == "cholesky":
            es.enter_context(gpts.max_cholesky_size(float("inf")))
        elif sampler == "ciq":
            ...
            es.enter_context(gpts.max_cholesky_size(0))
            es.enter_context(gpts.ciq_samples(True))
            es.enter_context(gpts.minres_tolerance(2e-3))  # 控制准确度和运行时间
            es.enter_context(gpts.num_contour_quadrature(15))
        elif sampler == "lanczos":
    ...
    with torch.no_grad():
        mvns = []
        for i in range(Y_torch.shape[1]):
            mvns.append(models[i](X_candidate_torch.to('cpu')))

        y_samples = []
        for i in range(Y_torch.shape[1]):
            # 从模型中采样目标函数和约束
            # y_samples.append(models[i].likelihood(mvns[i]).sample(torch.Size([batch_size])).t().cpu().detach())
            y_samples.append(models[i].likelihood(mvns[i]).sample(torch.Size([batch_size])).t().to(device).detach())

    # 从目标函数样本中选择最优解，并确保这些解满足约束
    X_cand_select = torch.ones((batch_size, X_candidate_torch.size(-1)))
    for i in range(batch_size):
        # 针对每个样本，选择使目标函数最小且满足约束的点
        feasible_mask = y_samples[1][:, i] <= 0  # 这里假设第一个维度是目标函数，其余是约束
        if feasible_mask.any():
            indbest = torch.argmin(y_samples[0][feasible_mask, i])
            X_cand_select[i, :] = X_candidate_torch[feasible_mask][indbest, :]
        else:
            # 如果没有候选解满足约束，选择约束值最接近满足的解
            indbest = torch.argmin(y_samples[1][:, i])
            X_cand_select[i, :] = X_candidate_torch[indbest, :]

        # 确保被选择的候选解不会被再次选择
        y_samples[0][indbest, :] = torch.inf

    return X_cand_select, models, [y_mean, y_std]
